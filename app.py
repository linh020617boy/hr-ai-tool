...
def ask_ai(prompt: str) -> str:
    # Thử nhiều model + cả 2 API version
    candidates = [
    ...
    for version, model in candidates:
        ...
        try:
            ...
            errors.append(f"{version}/{model}: {resp.status_code} → {resp.text[:200]}")
        except Exception as e:
            errors.append(f"{version}/{model}: {str(e)}")
    st.error("❌ Lỗi kết nối Gemini API. Chi tiết:")
    for err in errors:
        st.code(err)
    st.stop()
